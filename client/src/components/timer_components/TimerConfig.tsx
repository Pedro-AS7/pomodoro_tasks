...
export default function TimerConfig({ setConfig, closeConfig, lastValues }: TimerConfigProps) {
	...
	// Atualiza os valores das configurações de acordo com os valores anteriores para os inputs
	useEffect(() => {
		setSettings({
	...
	// Aplica as configurações ao timer
	const handleApply = () => {
		setConfig(
	...
	// Define um manipulador de entrada genérico para atualizar as configurações
	const inputSetHandler = (input: string, value: string) => {
	...
	const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>, input: string) => {
		const value = e.target.value
		if (parseInt(value, 10) > 999) {
			inputSetHandler(input, '999')
		} else {
			inputSetHandler(input, value)
		}
	}

	const handleInputBlur = (e: React.FocusEvent<HTMLInputElement>, input: string) => {
		const value = Number(e.target.value)
		if (value < 1 || isNaN(value)) {
			inputSetHandler(input, '1')
		}
	}
